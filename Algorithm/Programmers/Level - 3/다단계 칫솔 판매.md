...
1. 민호는 다단계 조직을 이용하여 칫솔을 판매하고 있습니다.

2. 판매원이 칫솔을 판매하면 그 이익이 피라미드 조직을 타고 조금씩 분배되는 형태의 판매망입니다.

3. 어느정도 판매가 이루어진 후, 조직을 운영하던 민호는 조직 내 누가 얼마만큼의 이득을 가져갔는지가 궁금해졌습니다.

4. 예를 들어, 민호가 운영하고 있는 다단계 칫솔 판매 조직이 아래 그림과 같다고 합시다.
...
aimie 의 추천인인 mary 는 2 원을 받지만 이것의 10% 는 원 단위에서 절사하면 배분할 금액이 없기 때문에 mary 는 2 원을 모두 가집니다.
...
1. 해당 enroll과 referral을 참고하여 트리구조를 만들고, 셀러가 해당 품목을 판매했을 때 amount를 분배해서 저장하는 방식을 구성한다.

2. referall이 - 이라는 부분은 센터가 곧 부모
...
class Solution {
    ...
    HashMap<String, String> sell = new LinkedHashMap<>();
    HashMap<String, Integer> sum = new LinkedHashMap<>();
    ...
    public int[] solution(String[] enroll, String[] referral, String[] seller, int[] amount) {
        ...
        ArrayList<Integer> answerList = new ArrayList<>(sum.values());
    ...
    void sell(String seller, int amount)
    {
        int fee = amount / 10;
        
        if(sell.containsKey(seller) && fee != 0) {
            sum.put(seller, sum.get(seller) + (amount - fee));
            sell(sell.get(seller), fee);
        } else {
            if(sum.containsKey(seller)) {
                sum.put(seller, sum.get(seller) + amount);
            }
        }
    }
}
...
아무래도 서치하면서 전 배열을 다 도는 부분이 문제가 있지 않을까 했었는데.. 

테스트 1 〉	통과 (0.27ms, 52.9MB)
테스트 2 〉	통과 (0.61ms, 52.4MB)
테스트 3 〉	통과 (0.59ms, 53.1MB)
테스트 4 〉	통과 (2.80ms, 53.5MB)
테스트 5 〉	통과 (20.01ms, 53.5MB)
테스트 6 〉	통과 (24.67ms, 62.1MB)
테스트 7 〉	통과 (66.69ms, 62.4MB)
테스트 8 〉	통과 (447.62ms, 67.6MB)
테스트 9 〉	통과 (4057.99ms, 66.8MB)
테스트 10 〉	실패 (시간 초과)
테스트 11 〉	실패 (시간 초과)
테스트 12 〉	실패 (시간 초과)
테스트 13 〉	실패 (시간 초과)
...
class Solution {
    ...
    public int[] solution(String[] enroll, String[] referral, String[] seller, int[] amount) {
        ...
        for (int i=0; i<n; i++) {
            list.add(new Node(enroll[i], 0));
            list.get(i).parent = new Node(referral[i], 0);
        }
    ...
    void sell(String seller, int amount)
    {
        ...
        for (int i=0; i<n; i++) {
            ...
            if(list.get(i).seller.equals(seller)) {
                
                if(list.get(i).parent != null && fee != 0) {
                    list.get(i).amount += amount - fee;
                    sell(list.get(i).parent.seller, fee);
                } else {
...
8할정도 온 느낌인데... 몇 개만 계산이 제대로 안 됐다. why? amount의 합을 구하지 않아서..
